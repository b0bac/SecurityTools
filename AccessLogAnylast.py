...
if __name__ == "__main__":
    ...
    if options.filepath == None:
        GetAllAccessLogFile(filepath=None)
    else:
        GetAllAccessLogFile(filepath=options.filepath)
    mylogfilelist = AccessLogFileList
    if options.count:
        if options.accesstime != None:
            for filename in mylogfilelist:
                GetAccessIPListByTime(filename,accesstime=options.accesstime)
        elif options.accessdate != None:
            for filename in mylogfilelist:
                GetAccessIPListByDate(filename,date=options.accessdate)
        else:
            for filename in mylogfilelist:
                GetAccessIPList(filename)
        exit(0)
    if options.detail:
